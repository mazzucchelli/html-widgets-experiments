...
export class ReactiveHtml {
  constructor({ components, events }) {
    ...
    this.rootElement = document.body.querySelector("[rh-root]");
    this.componentSelector = "[data-rh]";
  ...
  async importComponents(target) {
    ...
    names.forEach(async (compName, i) => {
      ...
      try {
        ...
        //   this.onNewComponentInstance(comp);
        // }
        const componentInstance = this.componentList[compName];
        // const componentInstance = await this.importerFun(compName);
        COMPONENT_LIST.set(element.CID, componentInstance);
        // this.onNewComponentInstance(componentInstance);

        if (!componentInstance) {
          console.warn("No Component found for", compName);
          return;
        }

        const [init] = componentInstance({ element, hooks });
        init();
      } catch (err) {
